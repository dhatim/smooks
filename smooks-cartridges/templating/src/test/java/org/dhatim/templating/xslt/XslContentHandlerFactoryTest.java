...
public class XslContentHandlerFactoryTest {
	...
	public void testXslUnitTrans_filebased_replace() {
		...
		SmooksResourceConfiguration res = new SmooksResourceConfiguration("p", "org/dhatim/templating/xslt/xsltransunit.xsl");
		...
		CharUtils.assertEquals("XSL Comparison Failure - See xsltransunit.expected1.", "/org/dhatim/templating/xslt/xsltransunit.expected1", transResult);
	}
	...
	public void testXslUnitTrans_parambased(String action, String expectedFileName) {
		...
		CharUtils.assertEquals("XSL Comparison Failure.  action=" + action + ".  See " + expectedFileName, "/org/dhatim/templating/xslt/" + expectedFileName, transResult);
	}
    ...
    public void test_badxsl() throws IOException, SAXException {
        ...
        try {
            ...
            assertEquals("Error loading Templating resource: Target Profile: [[org.dhatim.profile.Profile#default_profile]], Selector: [#document], Selector Namespace URI: [null], Resource: [/org/dhatim/templating/xslt/bad-stylesheet.xsl], Num Params: [0]", e.getCause().getMessage());
        }
    }
}
