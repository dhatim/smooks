...
/**
 * Sort Comparator for {@link org.dhatim.cdr.SmooksResourceConfiguration} Objects based on their "specificity".
 * <p/>
 * Before reading this be sure to read the {@link org.dhatim.cdr.SmooksResourceConfiguration} class Javadoc.
 * <p/>
 * As Smooks applies {@link org.dhatim.delivery.ContentHandler}s ({@link org.dhatim.delivery.dom.DOMElementVisitor DOMElementVisitors} and
 * {@link org.dhatim.delivery.dom.serialize.SerializationUnit SerializationUnits}) it may discover that in a given case more than 1 {@link org.dhatim.delivery.ContentHandler}
 * can be applied.  How does Smooks decide on the order in which these {@link org.dhatim.delivery.ContentHandler}s are to be applied to the content?
 * <p/>
 * At the moment, Smooks uses this class to calculate a "specificity" rating for each Content Delivery Resource based on its 
 * {@link org.dhatim.cdr.SmooksResourceConfiguration &lt;smooks-resource&gt;} configuration, and sorts them in decreasing order of specificity.
 * <p/>
 ...
 * <pre>
	...
	{@link org.dhatim.cdr.ProfileTargetingExpression}[] profileTargetingExpressions = resourceConfig.{@link org.dhatim.cdr.SmooksResourceConfiguration#getProfileTargetingExpressions() getProfileTargetingExpressions()};
	for(int i = 0; i < profileTargetingExpressions.length; i++) {
		specificity += profileTargetingExpressions[i].{@link org.dhatim.cdr.ProfileTargetingExpression#getSpecificity(org.dhatim.profile.ProfileSet) getSpecificity(profileSet)};
	}
	
	// Check the 'selector' attribute value.
	if(resourceConfig.{@link org.dhatim.cdr.SmooksResourceConfiguration#isXmlDef() isXmlDef()}) {
		specificity += 10;
	} else if(resourceConfig.{@link org.dhatim.cdr.SmooksResourceConfiguration#getSelector() getselector()}.equals("*")) {
		specificity += 5;
	} else {
	...
	if(resourceConfig.{@link org.dhatim.cdr.SmooksResourceConfiguration#getSelectorNamespaceURI() getSelectorNamespaceURI()} != null) {
		specificity += 10;
	}</pre>  
