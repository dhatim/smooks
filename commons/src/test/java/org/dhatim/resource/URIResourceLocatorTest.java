...
public class URIResourceLocatorTest {
    ...
    public void test_fileBasedResource() throws IllegalArgumentException, IOException, URISyntaxException {
        ...
        stream = locator.getResource("src/test/java/org/dhatim/resource/somefile.txt");
        assertNotNull(stream);

        // Try it again now with a non-default base URI...
        locator.setBaseURI(new URI("src/test"));
        stream = locator.getResource("java/org/dhatim/resource/somefile.txt");
        assertNotNull(stream);
    }
	...
	public void test_classpathBasedResource() throws IllegalArgumentException, IOException {
		...
		try {
			stream = locator.getResource("classpath:/org/dhatim/resource/someunknownfile.txt");
		...
		// Resource exists...
		stream = locator.getResource("classpath:/org/dhatim/resource/somefile.txt");
		assertNotNull(stream);

        // Resource exists - no scheme - should get it from the classpath ...
		stream = locator.getResource("/org/dhatim/resource/somefile.txt");
		assertNotNull(stream);
	}
